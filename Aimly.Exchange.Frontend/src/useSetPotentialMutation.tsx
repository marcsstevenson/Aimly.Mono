// The purpose of this file is to provide a hook which updates the About You values via a mutation
// Refer here as the template used for this hook:
// https://codesandbox.io/s/relay-sandbox-nxl7i?file=/src/useRenameTodoMutation.ts
// An example of this template's use can be found here:
// https://codesandbox.io/s/relay-sandbox-nxl7i?file=/src/Todo.tsx:470-491

import { commitMutation, useRelayEnvironment } from 'react-relay';
import {
  useSetPotentialMutation as mutationType,
  SetPotentialCommandInput,
  GetPotentialModelInput,
  useSetPotentialMutation$data,
} from '__generated__/useSetPotentialMutation.graphql';

import graphql from 'babel-plugin-relay/macro';

const def = graphql`
  mutation useSetPotentialMutation($input: SetPotentialCommandInput!) {
    setPotential(input: $input) {
      updatedCompanyProfileId
    }
  }
`;

export default function useSetPotentialMutation() {
  const environment = useRelayEnvironment();

  const SetPotential = (
    getPotentialModel: GetPotentialModelInput,
    onCompleted: (response: useSetPotentialMutation$data) => void
  ) => {
    const input: SetPotentialCommandInput = {
      getPotentialModel,
    };

    return commitMutation<mutationType>(environment, {
      mutation: def,
      variables: { input },
      onCompleted: (response) => {
        onCompleted(response);
      } /* Mutation completed */,
    });
  };

  return SetPotential;
}
